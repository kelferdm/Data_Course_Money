...
ggplot(MplsStops,aes(x=Neighborhood,y=Frequency, fill = Gender))+
...
## shows individual counts in each neighborhood
table(MplsStops$neighborhood)

neighborhoodnrace<-table(MplsStops$neighborhood, MplsStops$gender)

#change to data frame to plot
neighborrace <- as.data.frame(neighborhoodnrace)

#assign new column names to neighborrace data frame
names(neighborrace) <- c("Neighborhood", "Race", "Frequency")
pop<-neighborrace[neighborrace$Frequency > 500,]

MplsStops$neighborhood

library(dplyr)
?join
View(MplsDemo)
DemoStop <- full_join(MplsStops, MplsDemo, "neighborhood")

view(DemoStop)

ggplot(DemoStop, aes(x=poverty, y=collegeGrad))

ggplot(DemoStop, aes(x=poverty, y=collegeGrad, size=population))+
  geom_bar(stat="identity")


#TEST Assighment 2 and 3
##cp 
view(iris)
read.csv("../Data_Course/Data/thatch_ant.csv")




#hist(MplsStops$income), hist, boxplot, bar
hist(MplsStops$income)
hist(MplsStops$income, xlab = "Income",ylab = "Number of Stops", main = "Income Histogram")
ggsave("../Data_Course_Money/ggplot.jpeg", width = 8,height = 3,dpi = 600)
##where am I?
getwd()

#how to send jpeg to my data course file
jpeg("../Data_Course_Money/practiceplot.jpg")
hist(MplsStops$income)
dev.off()